...
export async function signUp(req: Request, res: Response, next: NextFunction) {
    let { contactNumber, fullname, email, role } = req.body;

    if (!contactNumber || !fullname || !email || !role) return res.status(400).json({ message: "Kindly send all required fields" })
    const existingUser = await userModal.find({ contactNumber : contactNumber });

    if (existingUser.length) return res.status(400).json({ message: "This Number is already registered!" })


    contactNumber = '+91' + contactNumber;
    let newUser = new userModal({ fullname, contactNumber, email, role })
    await newUser.save();
